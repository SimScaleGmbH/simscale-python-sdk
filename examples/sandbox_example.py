...
geometry_import = GeometryImportRequest(
    ...
    format='STL',
    input_unit='m',
    options=GeometryImportRequestOptions(facet_split=True, sewing=True, improve=True, optimize_for_lbm_solver=True),
)
...
model = WindComfort(
    ...
    wind_conditions=WindConditions(
        geographical_location=GeographicalLocation(latitude=DimensionalAngle(41.385064, "°"),
                                                   longitude=DimensionalAngle(2.173404, "°")),
        wind_rose=WindRose(
            num_directions=4,
...
results = simulation_run_api.get_simulation_run_results(project_id, simulation_id, run_id, category='STATISTICAL_SURFACE_SOLUTION')
